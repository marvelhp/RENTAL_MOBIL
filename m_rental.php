...
public function insert_detail($where)
	{
		$sql ="insert into detail_pinjam (id_pinjam,id_mobil,denda) 
		select transaksi.id_pinjam,transaksi.id_mobil,transaksi.denda from transaksi where transaksi.id_customer='$where'";
		$this->db->query($sql);
	}

  public function find($where, $table){
	//Query mencari record berdasarkan ID-nya
		$hasil = $this->db->where('id_mobil', $where)
						  ->limit(1)
						  ->get($table);
		if($hasil->num_rows() > 0){
			return $hasil->row();
